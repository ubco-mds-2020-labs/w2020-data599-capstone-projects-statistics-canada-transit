...
```{r}
# fill NA for existing origin IDs
...
filled_isochrone_frame <- NA_table_filler(isochrone_frame, isochrone = TRUE)
...
# check isochrones: there should be 4 counts per fromId
iso_id_counts <- all_isochrone_frame %>% group_by(fromId) %>% summarise(n = n())
unique(iso_id_counts$n)

# check scores: there should be 32 counts per fromId
...
for (amenity in type_name) { 
  for (add_stop in level) {

  # 4 isochrone maps

    map_maker_isochrone(data = isochrone_viz_frame_st,bus_data =station_locs,amenity,add_stop, output_dir = '../../data/html_maps/isochrone_maps')
    ...
    for (weight in unique(scores_viz_frame_st$weight)) {
      ...
      # 32 others
        map_maker_scores(data = scores_viz_frame_st,bus_data =station_locs, amenity,add_stop=add_stop, weight=weight, nearest_n,output_dir = '../../data/html_maps/score_maps')
      } 
    }
  }
}
