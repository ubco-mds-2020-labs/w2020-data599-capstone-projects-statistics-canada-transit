...
unique(ttm$toId)%>%length()
unique(destinations$id)%>%length()
...
unique(ttm$toId)[unique(ttm$toId) %in% unique(amenity_wts$id)]



unique(amenity_wts$id)%>%length()
id_in_ttm<-unique(ttm$toId)[unique(ttm$toId) %in% unique(amenity_wts$id)]

### subset these id that not in amenity wts
amenity_wts
ttm_sub<-subset(ttm, !(toId %in% id_in_ttm))
paste('Number of id that not in amenity wts ', unique(ttm_sub$toId)%>%length())
# assgin minium weight on those places
ttm_sub_id<-unique(ttm_sub$toId)
ttm_sub_id<-as.data.frame(ttm_sub_id)
ttm_sub_id$weight<-min(amenity_wts$weight)
colnames(ttm_sub_id)[1]<-"id"
ttm_sub_id
amenity_wts
amenity_wts<-rbind(amenity_wts,ttm_sub_id)
# are all the ttm amenity IDs in the weighted IDs set?
paste('Are all the ttm amenity IDs in the weighted IDs set? (needs to be true for the join to work)')
all(unique(ttm$toId) %in% unique(amenity_wts$id))

if (all(unique(ttm$toId) %in% unique(amenity_wts$id))==TRUE){
  # join when weight index is corrected
  destinations <- left_join(destinations, amenity_wts, by = c('id' = 'id'))
  ttm <-  left_join(ttm, destinations, by = c('toId' = 'id'))
...
scores_long<-rbind(scores_long_weighted,scores_long_unweighted)
...
sum(is.na(filled_isochrone_frame$type))
```
...
sum(is.na(isochrone_viz_frame_st$type))
map_maker_scores(scores_viz_frame_st, 'gallery',"no","1")
map_maker_isochrone(data = isochrone_viz_frame_st,"museum" )
sub
```
