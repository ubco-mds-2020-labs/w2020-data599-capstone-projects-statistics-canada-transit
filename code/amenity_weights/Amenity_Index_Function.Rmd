...
library_index<-read_csv("data/amenity_weights/Library_weights.csv")
...
```{R}
merged_data%>%select(poi_name,Name,open_days,Total_hours,Rating,Total_Review,lat,lon)->df

colnames(df)
colnames(df) <- c("POI Name", "Google Place Name","Open Days","Open Hours","Rating","Total Review","lat","lon")
df[df == 0] <- NA
...
weights<-function(data,Amenity="museuem"){
  data%>%filter(type==Amenity)->poi_type
  # select relevent features 
  poi_type%>%select(poi_name,open_days,Total_hours,Rating,Total_Review,lat,lon)->poi_type
  # check number of missing data
  poi_type[poi_type == 0] <- NA
  missing_percentage<-colMeans(is.na(poi_type))
