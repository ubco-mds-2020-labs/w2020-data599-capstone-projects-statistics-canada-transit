...
rollingAvg_prox <- function(row){
  prox_score <- 1+filter(scores_func, scores_func$lat <= (as.numeric(row["lat"])+0.005) & scores_func$lat >= (as.numeric(row["lat"])-0.005) & scores_func$lon <= (as.numeric(row["lon"])+0.005) & scores_func$lon >= (as.numeric(row["lon"])-0.005)) %>% select(9:18) %>% colMeans(na.rm = TRUE) %>% mean()
  if(is.na(prox_score)){
    prox_score <- 1
  }
  return(prox_score)
...
# Get the most recent trafic survey for each row
...
```{r}
scores_pos[,c(1,3)] %>% group_by(fromId) %>% summarise(mean_score = mean(score))
```
...
scores_prox_score$scoreCorrection <- normalize_vec(scores_prox_score$scoreCorrection)+1

scores_prox_score$eff <- normalize_vec_nonAbs(scores_prox_score$pop_norm_squared/scores_prox_score$prox_score - scores_prox_score$score/scores_prox_score$scoreCorrection)
...
for(df in df_types_eff){
  scores_func <- df
  df$eff_ravg <- 1-normalize_vec_nonAbs(apply(df,1, rollingAvg))
  scores_eff_all <- rbind(scores_eff_all,df)
}
efficiency_frame <- scores_eff_all %>% select(1:3,6:8,20,22,23)
```
...
#                       output_dir = '../../data/html_maps/efficiency_maps_quant')
  map_maker_efficiency_quant(data = eff_viz_frame_st, amenity,# view_map = TRUE,
                       output_dir = '../../data/html_maps/efficiency_maps_corr')
