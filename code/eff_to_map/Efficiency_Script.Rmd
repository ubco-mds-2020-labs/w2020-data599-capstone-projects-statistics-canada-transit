...
scores_long_pos <-filter(scores_long,stringr::str_detect(nearest_n, 'all') & stringr::str_detect(weight, 'no'))[,c(1,3)] %>% group_by(fromId) %>% summarise(mean_score = mean(score))
...
scores_prox_score$eff <- normalize_vec_nonAbs(scores_prox_score$prox_score - scores_prox_score$mean_score/scores_prox_score$scoreCorrection)
