...
near1 <- ttm %>%
  ...
  summarise(avg_time = min(avg_time),
            Dest = toId[which.min(avg_time)],
            # t1 = t1[which.min(avg_time)],
            Latitude = lat[which.min(avg_time)],
            Longitude = lon[which.min(avg_time)])
...
## Export
write.csv(near1, 'Nearest1_719.csv', row.names = FALSE)
```
...
amenity_wts <- read.csv('amenity_wts.csv')
...
ttm_full %>% group_by(time_group) %>% summarise(n = n()) -> t_groups
