...
#Preprocessing for effiency scores
```{r}

# Function to calculate effiency withing 'X' degrees of each block
  func <- function(row){
    mean(filter(scores_func, scores_func$lat <= (as.numeric(row["lat"])+0.02) & scores_func$lat >= (as.numeric(row["lat"])-0.02) & scores_func$lon <= (as.numeric(row["lon"])+0.02) & scores_func$lon >= (as.numeric(row["lon"])-0.02))$eff)
  }
...
#scores_eff_all <- scores_eff_all[-c(3,4,5)]

#scores_eff_all <- rbind(scores_eff_gal,scores_eff_lib,scores_eff_thea,scores_eff_mus)[-c(3,4,5)]
write.csv(scores_eff_all[,c(1,2,6:8,10)], '../../data/efficiency/running_eff_scores.csv', row.names = FALSE)
```

```{r}
scores_eff_all[,c(1,2,6:8,10)]
```
