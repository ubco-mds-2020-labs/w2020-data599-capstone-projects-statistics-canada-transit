...
```{r}
# Mapping function
map_maker_scores <- function(data, amenity, weight, nearest_n, output_dir) {
  ...
  print(paste('Current Map:', glue('{amn_name} - wt({weight}) - n({str_to_upper(nearest_n)})')))
  ...
  map
  #mapshot(map, url = paste0(getwd(), glue("{output_dir/{file_name}.html")))

}



map_maker_isochrone <- function(data, amenity, output_dir) {
  ...
  print(paste('Current Map:', glue('{amn_name} - isochrone')))
  ...
  p_popup <- paste0("<strong>Nearest:<strong>", amn_name,"</strong>", 
                    "<br><strong>Travel Time: ", time_groups, "</strong>")
