...
# 
# df%>%gather(key = "key", value = "val",-type) %>%mutate(isna = is.na(val))%>%
#   mutate(isna = is.na(val)) %>%
#   group_by(type) %>%
#   mutate(total = n()) %>%
#   group_by(type, total, isna) %>%
#   summarise(num.isna = n()) %>%
#   mutate(pct = num.isna / total * 100)%>%filter(type %in% c("museum","gallery","theatre/performance and concert hall","library or archives"))->missing_poi


# 
# levels <-(missing_poi  %>% filter(isna == T) %>% arrange(desc(pct)))$type
# 
# label_txt<-missing_poi%>% filter(isna == T)%>%select(pct)
# 
...
# missing_poi%>%
#       ggplot() +
#         geom_bar(aes(x = reorder(type, desc(pct)), 
#                      y = pct, fill=isna), 
#                  stat = 'identity', alpha=0.8) +
#      geom_text(data=label_txt,aes(label =pct,x= reorder(type, desc(pct)),y=pct), size = 3,hjust=-1.1)+
...
```{R}
#open_days+Total_hours+Rating+Total_Review
...
f1<-function(x) {sum(!is.na(x))/length(x)}
df
agg_type%>%select(-poi_name,-lat,-lon)
