...
# Mapping function
map_maker <- function(data, amenity, weight, nearest_n) {
  
  amn_name <- amenity %>% str_to_title() %>% str_replace_all('Or', 'or') %>% str_replace('And', 'and')
  
  # subset info
  polyg_subset <- data %>% filter(type == amenity, weight == weight, nearest_n == nearest_n)
  ...
  # popup
  percentile <- ecdf(data$score)
  p_popup <- paste0("<h5>Accessibility Percentile: ", percentile(data$score),"</h5>",
                    "<br><strong>Block ID: ", data$DBUID,"</strong>",
                    "<br><strong>Block Pop.: ", data$DBUID,"</strong>",
                    "<br><strong>Raw Score: ", round(data$score, 3),"<strong>")
  ...
  map <- leaflet(data = polyg_subset) %>%
    ...
    addLegend("bottomleft",  # location
              ...
              title = glue('Vancouver {amn_name} Transit Accessibility; Weighted ({weight}), Nearest_n ({str_to_upper(nearest_n)})'))
  
  map
}

map_maker(data = van_dbs_scores_st, 'library or archives', 'yes', '1')

for (amenity in unique(allblocks$type)) {
  for (weight in unique(allblocks$weight)) {
    for (nearest_n in unique(allblocks$nearest_n)) {
      
      amn_name <- amenity %>% str_to_title() %>% str_replace_all('Or', 'or') %>% str_replace('And', 'and')
      title <- glue('Vancouver {amn_name} Transit Accessibility. Weighted: {weight}, Nearest: {nearest_n}')
      print(title)
      
      temp_map <- map_maker(data = van_dbs_scores_st, amenity, weight, nearest_n)
      mapshot(temp_map, url = paste0(getwd(), glue("New HTML Maps/{title}.html")))
    }
  }
}
...
```{r}
data = van_dbs_scores_st
amenity = 'library or archives'
weight = 'yes'
nearest_n = '1'
