...
Note that the new scoring function is a much more realistic way of computing the worst case accessibility score. This is because we add 2* standard deviation in travel time to the average travel time as a way of getting a worst case travel time. The original method multiplies travel time with standard deviation which doesn't exactly make sense since both those variables don't have the same weight.

**The score output from this script will thus be the product of the new scoring function below**.
...
This is done to prevent infinity values in the scoring. Normalization will be done to prevent zero values but it still creates a largely skewed score if we include travel times that approach zero. 1 minute is also a realistic time window for any travel time that may take 0 - 1 minute.
...
```{r}
# Score we care about
ttm_scores <- sum_score_fxn(ttm_wts, weight = FALSE)
ttm_wtd_scores <- sum_score_fxn(ttm_wts, weight = TRUE)
...
ttm_scores <- sum_score_fxn(ttm_wts, weight = FALSE, log_normalize_score = FALSE)

ttm_wtd_scores <- sum_score_fxn(ttm_wts, weight = TRUE, log_normalize_score = FALSE)


plot_densities(ttm_scores, ttm_wtd_scores, 'Norm Score / Unweighted / No df Normalization', 'Norm Score / Weighted / No df Normalization')
...
n1_ttm_score <- sum_score_fxn(nearest_1_ttm, weight = FALSE)
n1_wt_ttm_score <- sum_score_fxn(nearest_1_ttm, weight = TRUE)

n2_ttm_score <- sum_score_fxn(nearest_2_ttm, weight = FALSE)
n2_wt_ttm_score <- sum_score_fxn(nearest_2_ttm, weight = TRUE)

n3_ttm_score <- sum_score_fxn(nearest_3_ttm, weight = FALSE)
n3_wt_ttm_score <- sum_score_fxn(nearest_3_ttm, weight = TRUE)
...
db_info <- read.csv('vancouver_db.csv')[1:2]
# convert to factor
...
All zones would ideally have 32 different scores based on possible schemes.
...
## Export
write.csv(long_scores, 'updated_long_scores.csv', row.names = FALSE)
